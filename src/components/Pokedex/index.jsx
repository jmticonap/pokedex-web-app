...
import { localDb, cardBackgoundStyle, capitalize } from "../../utils";
        ...
        <div className="card-container">
          <div className={`card__header ${getStyleByKey("head")}`}>
            <div className="card__header-inner">
              <img onLoad={imgLoadedHandler} src={data.image} alt="imagen" />
              {imgLoaded || (
                <Skeleton
                  variant="circular"
                  width={140}
              ...
              <div className="card__body__grid">
                <div>
                  ...
                  <h3 className={getStyleByKey("color")}>
                    {data.stats.find((i) => i.name === "hp")["value"]}
                  </h3>
                </div>
                <div>
                  <h6 className="subtitle_color">ATTACK</h6>
                  <h3 className={getStyleByKey("color")}>
                    {data.stats.find((i) => i.name === "attack")["value"]}
                  </h3>
                </div>
                <div>
                  <h6 className="subtitle_color">DEFENSE</h6>
                  <h3 className={getStyleByKey("color")}>
                    {data.stats.find((i) => i.name === "defense")["value"]}
                  </h3>
                </div>
                <div>
                  <h6 className="subtitle_color">SPEED</h6>
                  <h3 className={getStyleByKey("color")}>
                    {data.stats.find((i) => i.name === "speed")["value"]}
                  </h3>
                </div>
              </div>
            </div>
...
export const Pokedex = () => {
  ...
  const renderPokemonTypeItems = () => {
    if (listTypeUrl?.length > 0) {
      const _pokemonTypeList = [
        { name: "Explor All", url: "*" },
        ...listTypeUrl,
      ];
      return _pokemonTypeList.map((itm) => (
        <MenuItem key={itm.name} value={itm.url}>
          {itm.name}
        </MenuItem>
      ));
    }
  ...
  return (
    <div className="pokedex-container">
      ...
      <Pagination
        renderItem={(item) => (
          <PaginationItem sx={paginatorBtnStyle} {...item} />
        )}
